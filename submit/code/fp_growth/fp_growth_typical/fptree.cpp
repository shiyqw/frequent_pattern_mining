...
bool fp_Tree::exist_node() {
	...
	for(int i = 0 ; i < stree.branch.size() ; ++i) {
		if(stree.branch[i] != 0) return true;
	}
	return false;
}
...
void fp_Tree::mine() {
	...
	vector<int> t_branch = stree.branch;
	vector<int> t_base_count = stree.base_count;
...
void fp_Tree::st_update(int x) {
	...
	vector<int> t_branch;
	vector<int> t_base_count;
	vector<int> t_count(item_ord , 0);

	for(int i = 0 ; i < stree.branch.size() ; ++i) {
		int now = stree.branch[i];
		int p = tree[now].ahead;
		while(now != 0 && tree[now].ord != x) now = tree[now].ahead;
		if(tree[now].ord == x) {
			t_branch.push_back(tree[now].ahead);
			t_base_count.push_back(stree.base_count[i]);
			int t_now = now;
			while (tree[t_now].ahead != 0) {
				t_count[tree[tree[t_now].ahead].ord] 
					+= stree.base_count[i];
				t_now = tree[t_now].ahead;	
			}
		}
